...
RSpec.describe MXNet::Gluon::SymbolBlock do
  ...
  describe '.new' do
    ...
    it 'does not make the free input into a param' do
      expect(layer.params.has_key?(:i)).to eq(false)
    end

    it 'makes the weight and bias into params' do
      expect(layer.params['foo0_w']).to be_a(MXNet::Gluon::Parameter)
      expect(layer.params['foo0_b']).to be_a(MXNet::Gluon::Parameter)
    end
  end
