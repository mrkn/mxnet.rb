...
RSpec.describe MXNet::Gluon::Data::BatchSampler do
  context 'with `last_batch: :keep`' do
    let(:sampler) do
      sampler = MXNet::Gluon::Data::SequentialSampler.new(10)
      described_class.new(sampler, 3, last_batch: :keep)
    end
  ...
  context 'with `last_batch: :discard`' do
    let(:sampler) do
      sampler = MXNet::Gluon::Data::SequentialSampler.new(10)
      described_class.new(sampler, 3, last_batch: :discard)
    end
  ...
  context 'with `last_batch: :rollover`' do
    let(:sampler) do
      sampler = MXNet::Gluon::Data::SequentialSampler.new(10)
      described_class.new(sampler, 3, last_batch: :rollover).tap do |sampler|
